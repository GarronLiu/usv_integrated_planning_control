...
class MotorControlNode:
    def __init__(self):
        ...
        self.angle_threshold = 20  # Z型实验首向角偏转切换角度阈值
    ...
    def rc_out_callback(self, msg):
        if self.step_left == None:
            ...
            self.step_left = (self.left_pwm_max - self.pwm_mid) / self.duration / 100
            self.step_right = (self.right_pwm_max - self.pwm_mid) / self.duration / 100
        ...
        while not rospy.is_shutdown():
            ...
            if stage == 0:
                if time.time() - start_time < 10:
                    msg = OverrideRCIn()
                    msg.channels = [2000, 0, 2000] + [0] * 15  # 将所有通道设置为相同的值
                    self.pub.publish(msg)
                elif time.time() - start_time < 20:
                    msg = OverrideRCIn()
                    msg.channels = [1100, 0, 1100] + [0] * 15  # 将所有通道设置为相同的值
                    self.pub.publish(msg)
            ...
            #左回旋螺线实验
                ...
                if self.spiral_change_flag:
                    ...
                    if self.left_pwm < 1900:
                        ...
                        continue
                msg = OverrideRCIn()
                msg.channels = [self.left_pwm, 0, self.right_pwm] + [
                        0
                    ] * 15  # 将所有通道设置为相同的值
                self.pub.publish(msg)
