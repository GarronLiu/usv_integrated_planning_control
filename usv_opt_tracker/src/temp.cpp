...
public:
    ...
    ControlInterface(ros::NodeHandle &nh):node_(nh){
        ...
        reference_traj_sub_ = node_.subscribe<nav_msgs::Path>("/trajectory",1,&ControlInterface::trajCbk,this,ros::TransportHints().tcpNoDelay());
        odom_sub_ = nh.subscribe<nav_msgs::Odometry>("/odom",1,&ControlInterface::odomCbk,this,ros::TransportHints().tcpNoDelay());
        opt_tracker_.reset(new usv_system::UsvOptTracker());

        // control_timer_ = node_.createTimer(ros::Duration(1/control_frequency),&ControlInterface::controller_loop,this);
        if(cmd_type == 0){
            cmd_fcu_pub_ = node_.advertise<mavros_msgs::PositionTarget>("/mavros/setpoint_raw/local", 50);
        }else{
            rc_override_pub_ = node_.advertise<mavros_msgs::OverrideRCIn>("/mavros/rc/override",50);
        }
        traj_horizon_pub_ = node_.advertise<nav_msgs::Path>("tracker/trajectory/horizon", 50);
        traj_predict_pub_ = node_.advertise<nav_msgs::Path>("tracker/trajectory/nmpc_predict",50);
    ...
    void trajCbk(const nav_msgs::PathConstPtr& msg){
        ...
        if(length == 0){
            ...
            double vel_max = opt_tracker_->getMaxVelocity();
    ...
    void controller_loop(){
        ...
        while (ros::ok())
        {
            ...
            if(current_ref_traj.poses.empty()) continue;
            
            auto t1 = ros::Time::now().toNSec();
            if(current_ref_traj.poses.size()>1){
                // truncate path
                int min_id = -1;
            ...
            auto control_opt = opt_tracker_->getMPCControl(x_state_now,x_pred_traj);

            visPredTraj(x_pred_traj);

            control_state(0) = x_pred_traj[2](8);
            control_state(1) = x_pred_traj[2](9);
    ...
    /* For NMPC control: Velocity and Yaw Rate Control Command in local enu */
        ...
        // reference https://ardupilot.org/dev/docs/mavlink-rover-commands.html?highlight=guided#mavlink-rover-commands-set-attitude-target
    }
    ...
    void visPredTraj(const ct::core::StateVectorArray<usv::UsvDynamicModel::STATE_DIM> &traj_predict_vec_){
            if(traj_predict_pub_.getNumSubscribers()==0) return;
                nav_msgs::Path traj;
