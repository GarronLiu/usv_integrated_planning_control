...
public:
  ...
  /* main API */
  void            setEnvironment(const std::shared_ptr<VehicleCentriMap> &map_);
...
private:
  ...
  std::shared_ptr<VehicleCentriMap> vc_map_;
  // main input
...
public:
  ...
  static double costFunction(void* func_data, const Eigen::VectorXd& x, Eigen::VectorXd& grad);
  void          combineCost(const Eigen::VectorXd& x, Eigen::VectorXd& grad, double& cost);
  static int monitorProgress(void *instance,const Eigen::VectorXd &x,const Eigen::VectorXd &g,const double fx,
