...
void BsplineOptimizer::setParam(ros::NodeHandle& nh) {
  ...
  nh.param("optimization/lambda3", lambda3_, 0.0001);
  nh.param("optimization/lambda4", lambda4_, 0.01);
  nh.param("optimization/lambda5", lambda5_, 0.0);
  nh.param("optimization/lambda6", lambda6_, 0.0);
  nh.param("optimization/lambda7", lambda7_, 0.0);
  nh.param("optimization/lambda8", lambda8_, 0.0);
...
void BsplineOptimizer::setEnvironment(const std::shared_ptr<VehicleCentriMap> &map_) {
  this->vc_map_ = map_;
}

void BsplineOptimizer::evaluateEDTwithGrad(const Eigen::Vector3d& pos, double& dist, Eigen::Vector3d& grad){
  //获得以当前点pos为中心的正方形的四个角点
  Eigen::Vector2d pos_2d(pos(0),pos(1));
  vc_map_->evaluateEDTWithGrad(pos_2d,dist,grad);
}
...
/*   nlopt::opt opt(nlopt::algorithm(isQuadratic() ? algorithm1_ : algorithm2_), variable_num_);
  ...
  lbfgs::lbfgs_parameter_t params;
    ...
    params.past = 3;
    params.delta = 1.0e-8;
    params.max_iterations = max_iteration_time_[max_time_id_];
...
/*   if (dim_ != 1) {
  ...
  try {
    ...
    int ret = lbfgs::lbfgs_optimize(q,
                                  final_cost,
                                  BsplineOptimizer::costFunction,
                                  nullptr,
                                  BsplineOptimizer::monitorProgress,
                                  this,
                                  params);
...
void BsplineOptimizer::calcDistanceCost(const vector<Eigen::Vector3d>& q, double& cost,
  ...
  for (int i = order_; i < end_idx; i++) {
    evaluateEDTwithGrad(q[i], dist, dist_grad);
    if (dist_grad.norm() > 1e-4) dist_grad.normalize();//梯度单位化，表示下降方向；
...
void BsplineOptimizer::combineCost(const Eigen::VectorXd& x, Eigen::VectorXd& grad,
                                   double& f_combine) {
  /* convert the NLopt format vector to control points. */
  //将NLOPT中的待优化变量x转换成控制点g_q_
  ...
  for (int i = 0; i < order_; i++) {
    ...
    for (int j = dim_; j < 3; ++j) {
      g_q_[i][j] = 0.0;//用于支持1维到3维的b样条曲线优化
    }
  }
  //再填充free 变量
  for (int i = 0; i < variable_num_ / dim_; i++) {
    ...
    for (int j = dim_; j < 3; ++j) {
      g_q_[i + order_][j] = 0.0;//同366行注释
    }
  }
  //如果末端控制点为终点，则将ENDPOINT替换掉，因为无需对终点处的控制点进行优化
  if (!(cost_function_ & ENDPOINT)) {
    for (int i = 0; i < order_; i++) {
...
double BsplineOptimizer::costFunction(void* func_data, const Eigen::VectorXd& x, Eigen::VectorXd& grad){
  BsplineOptimizer* opt = reinterpret_cast<BsplineOptimizer*>(func_data);
  double            cost;
  opt->combineCost(x, grad, cost);
  opt->iter_num_++;
...
int BsplineOptimizer::monitorProgress(void *instance,
